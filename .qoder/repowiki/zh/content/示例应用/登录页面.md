...
**更新** 登录页面已完成重大重构，从ChatUIKit/modules/Login组件迁移到vue2-demo/pages/login/index.vue独立页面，增加了令牌登录和密码登录双模式、表单验证、加载状态、错误处理等功能，代码行数从约100行增加到382行。
...
I["index.ts<br/>ChatUIKit 单例与 Store 访问"]
J["stores/chat.ts<br/>聊天状态与登录入口"]
K["stores/conn.ts<br/>连接管理"]
L["stores/appUser.ts<br/>用户信息与在线状态"]
M["sdk.ts<br/>SDK 封装"]
end
subgraph "Demo示例应用"
...
- **Store依赖**：
  - ChatStore依赖ConnStore获取连接实例，依赖AppUserStore通过SDK事件回调拉取用户信息和在线状态
  - ConnStore依赖SDK封装以创建连接实例
- **国际化依赖**：
...
**更新** 故障排查指南已更新，增加了对SDK版本显示和国际化配置的检查指导。
