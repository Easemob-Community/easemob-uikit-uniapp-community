# 用户状态管理

<cite>
**本文档引用的文件**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts)
- [ChatUIKit/stores/conn.ts](file://ChatUIKit/stores/conn.ts)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts)
- [ChatUIKit/types/index.ts](file://ChatUIKit/types/index.ts)
- [ChatUIKit/sdk.ts](file://ChatUIKit/sdk.ts)
- [ChatUIKit/const/index.ts](file://ChatUIKit/const/index.ts)
- [ChatUIKit/components/Avatar/index.vue](file://ChatUIKit/components/Avatar/index.vue)
- [demo/pages/Login/index.vue](file://demo/pages/Login/index.vue)
- [demo/pages/Me/index.vue](file://demo/pages/Me/index.vue)
- [demo/pages/Profile/index.vue](file://demo/pages/Profile/index.vue)
- [demo/pages/ProfileSetting/index.vue](file://demo/pages/ProfileSetting/index.vue)
- [demo/pages/PresenceSetting/index.vue](file://demo/pages/PresenceSetting/index.vue)
- [ChatUIKit/log.ts](file://ChatUIKit/log.ts)
</cite>

## 更新摘要
**变更内容**
- 更新以反映appUser store的重大增强：包括fetchingUserIds去重机制、getUserInfo getter增强、getUsersInfoFromServer批量获取功能等
- 优化用户信息获取性能，通过智能去重避免重复请求
- 增强getter功能，提供更完整的用户信息组合
- 扩展批量获取能力，支持一次性处理多个用户ID
- **新增** 改进updateUserInfo动作，自动同步更新userInfoMap，确保用户信息一致性

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性地文档化了用户状态管理 Store 的设计与实现，覆盖当前用户信息、在线状态与偏好设置的管理机制；阐述用户登录、登出与会话保持流程；解释用户状态与连接状态的关联关系及安全验证要点；并给出用户状态持久化与隐私保护策略建议。目标是帮助开发者快速理解并正确使用用户状态管理能力。

**更新** 本次更新重点反映了appUser store的重大增强：采用了fetchingUserIds去重机制，通过智能过滤避免重复请求；getUserInfo getter得到显著增强，提供更完整的用户信息组合；getUsersInfoFromServer支持批量获取功能，大幅提升多用户场景下的性能表现；**新增** updateUserInfo动作现在会自动同步更新userInfoMap，确保用户信息一致性。

## 项目结构
围绕用户状态管理的关键文件组织如下：
- 存储层：Pinia Store（用户状态、聊天状态、连接状态、配置）
- 类型与常量：统一的类型定义与状态枚举
- UI 层：登录、个人中心、在线状态设置页面
- 组件层：头像组件，基于用户在线状态渲染角标
- SDK 封装：对 Easemob Web SDK 的统一封装

```mermaid
graph TB
subgraph "存储层"
AU["AppUserStore<br/>用户信息/在线状态"]
CS["ChatStore<br/>登录/登出/事件"]
CONN["ConnStore<br/>连接实例"]
CFG["ConfigStore<br/>功能/主题配置"]
end
subgraph "类型与常量"
T["types/index.ts<br/>UserInfoWithPresence/PresenceInfo"]
C["const/index.ts<br/>PRESENCE_STATUS_LIST"]
end
subgraph "UI层"
L["Login 页面"]
M["Me 页面"]
PS["PresenceSetting 页面"]
PR["Profile 页面"]
PRSET["ProfileSetting 页面"]
end
subgraph "组件层"
AV["Avatar 组件"]
end
subgraph "SDK封装"
S["sdk.ts<br/>chatSDK"]
LOG["log.ts<br/>logger"]
end
L --> CS
M --> AU
PS --> AU
PR --> AU
PRSET --> AU
AV --> AU
CS --> CONN
CS --> CFG
AU --> CONN
AU --> CFG
CONN --> S
AU --> T
PS --> C
CS --> LOG
AU --> LOG
```

**图表来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L1-L242)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L1-L407)
- [ChatUIKit/stores/conn.ts](file://ChatUIKit/stores/conn.ts#L1-L84)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L1-L124)
- [ChatUIKit/types/index.ts](file://ChatUIKit/types/index.ts#L67-L80)
- [ChatUIKit/const/index.ts](file://ChatUIKit/const/index.ts#L17-L25)
- [ChatUIKit/sdk.ts](file://ChatUIKit/sdk.ts#L1-L13)
- [ChatUIKit/components/Avatar/index.vue](file://ChatUIKit/components/Avatar/index.vue#L1-L188)
- [demo/pages/Login/index.vue](file://demo/pages/Login/index.vue#L165-L302)
- [demo/pages/Me/index.vue](file://demo/pages/Me/index.vue#L67-L88)
- [demo/pages/Profile/index.vue](file://demo/pages/Profile/index.vue#L1-L116)
- [demo/pages/ProfileSetting/index.vue](file://demo/pages/ProfileSetting/index.vue#L1-L101)
- [demo/pages/PresenceSetting/index.vue](file://demo/pages/PresenceSetting/index.vue#L94-L147)
- [ChatUIKit/log.ts](file://ChatUIKit/log.ts#L1-L78)

## 核心组件
- AppUserStore：集中管理用户信息与在线状态，提供获取、更新、订阅/取消订阅在线状态、发布自定义在线状态等能力。**更新**：采用fetchingUserIds去重机制，通过智能过滤避免重复请求；getUserInfo getter增强，提供更完整的用户信息组合；getUsersInfoFromServer支持批量获取功能；**新增** updateUserInfo动作自动同步更新userInfoMap，确保用户信息一致性。
- ChatStore：负责登录、登出、SDK事件初始化与分发、初始数据加载；与 AppUserStore 协作拉取当前用户信息与在线状态。
- ConnStore：统一持有与暴露 IM SDK 连接实例，提供初始化、关闭、清理等能力。
- ConfigStore：统一管理功能开关与主题配置，如用户信息与在线状态功能开关。
- 类型与常量：定义用户信息与在线状态的数据结构、支持的在线状态枚举。
- UI 组件与页面：登录、个人中心、在线状态设置页面，以及头像组件展示在线状态。

**章节来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L17-L242)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L22-L398)
- [ChatUIKit/stores/conn.ts](file://ChatUIKit/stores/conn.ts#L15-L84)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L14-L124)
- [ChatUIKit/types/index.ts](file://ChatUIKit/types/index.ts#L67-L80)
- [ChatUIKit/const/index.ts](file://ChatUIKit/const/index.ts#L17-L25)

## 架构总览
用户状态管理贯穿"登录—连接—状态同步—UI渲染"的完整链路，关键交互如下：

```mermaid
sequenceDiagram
participant U as "用户"
participant L as "Login 页面"
participant CS as "ChatStore"
participant CONN as "ConnStore"
participant AU as "AppUserStore"
participant CFG as "ConfigStore"
U->>L : "输入账号/密码或短信验证码"
L->>CS : "login({user,pwd|accessToken})"
CS->>CONN : "open(params)"
CS->>CS : "initSDKEvent()"
CS->>AU : "getUsersInfoFromServer([user])"
CS->>AU : "getUsersPresenceFromServer([user])"
CS->>CS : "loadInitialData()"
CS-->>U : "跳转会话列表/完成登录"
```

**图表来源**
- [demo/pages/Login/index.vue](file://demo/pages/Login/index.vue#L165-L302)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L299-L328)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L86-L125)

## 详细组件分析

### 用户信息与在线状态数据模型
- 用户信息结构：包含昵称、头像、签名等；通过 getter 组合展示名称与头像默认值。
- 在线状态结构：包含是否在线与扩展字段（如预设状态或自定义描述）。
- 统一视图：UserInfoWithPresence 将用户属性与在线状态合并，便于 UI 直接消费。

```mermaid
classDiagram
class AppUserState {
+Record~string,UserInfo~ userInfoMap
+Record~string,PresenceInfo~ userPresenceMap
}
class PresenceInfo {
+boolean isOnline
+string presenceExt
}
class UserInfoWithPresence {
+string name
+string nickname
+string avatar
+string sign
+string presenceExt
+boolean isOnline
}
AppUserState --> PresenceInfo : "用户在线状态映射"
UserInfoWithPresence --> PresenceInfo : "组合"
```

**图表来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L17-L22)
- [ChatUIKit/types/index.ts](file://ChatUIKit/types/index.ts#L67-L80)

**章节来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L17-L80)
- [ChatUIKit/types/index.ts](file://ChatUIKit/types/index.ts#L67-L80)

### AppUserStore：用户状态管理核心
- 状态结构：以对象替代 Map，提升 Vue 响应式兼容性。
- Getter：
  - 获取用户信息（带默认名与头像兜底）
  - 获取当前登录用户信息
  - 获取用户在线状态
  - 检查是否已缓存用户信息
- Actions：
  - 从服务器批量获取用户属性（按需过滤已缓存）
  - 获取用户在线状态（解析在线标志位）
  - 订阅/取消订阅用户在线状态
  - **保留** 发布自定义在线状态
  - 设置/更新用户信息
  - 清空所有用户数据

**更新** 采用fetchingUserIds去重机制，通过智能过滤避免重复请求；getUserInfo getter增强，提供更完整的用户信息组合；getUsersInfoFromServer支持批量获取功能，大幅提升多用户场景下的性能表现；**新增** updateUserInfo动作现在会自动同步更新userInfoMap，确保用户信息一致性。

```mermaid
flowchart TD
Start(["进入 AppUserStore"]) --> CheckFeature["检查功能开关 useUserInfo/usePresence"]
CheckFeature --> FetchUsers{"是否传入 userIdList？"}
FetchUsers --> |否| ReturnEmpty["返回空结果"]
FetchUsers --> |是| FilterCache["过滤已缓存用户ID"]
FilterCache --> HasFetch{"是否有待获取用户？"}
HasFetch --> |否| ReturnCache["返回已缓存结果"]
HasFetch --> |是| CallSDK["调用 SDK 接口获取用户信息/在线状态"]
CallSDK --> UpdateState["写入 userInfoMap/userPresenceMap"]
UpdateState --> End(["结束"])
ReturnEmpty --> End
ReturnCache --> End
```

**图表来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L86-L125)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L130-L159)

**章节来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L17-L242)

### ChatStore：登录、登出与会话保持
- 登录：
  - 打开连接
  - 初始化 SDK 事件
  - 加载初始数据（会话、联系人、群组）
  - 拉取当前用户信息与在线状态
- 登出：
  - 关闭连接
  - 清空各 Store 状态
  - 重置连接状态
- 会话保持：
  - 在 onShow 生命周期触发 SDK 的 onShow，维持连接活跃

```mermaid
sequenceDiagram
participant CS as "ChatStore"
participant CONN as "ConnStore"
participant AU as "AppUserStore"
participant CFG as "ConfigStore"
CS->>CONN : "open(params)"
CS->>CS : "initSDKEvent()"
CS->>CFG : "读取功能开关"
CS->>AU : "getUsersInfoFromServer([user])"
CS->>AU : "getUsersPresenceFromServer([user])"
CS->>CS : "loadInitialData()"
CS-->>CS : "setConnState('connected')"
```

**图表来源**
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L299-L328)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L377-L396)

**章节来源**
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L29-L51)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L299-L396)

### ConnStore：连接实例管理
- 提供类型安全的连接实例访问
- 初始化连接（若未初始化）
- 关闭连接并清空状态

**章节来源**
- [ChatUIKit/stores/conn.ts](file://ChatUIKit/stores/conn.ts#L15-L84)

### ConfigStore：功能与主题配置
- 默认开启用户信息与在线状态功能
- 支持隐藏/显示特定功能
- 提供资源 URL 与主题配置

**章节来源**
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L14-L124)
- [ChatUIKit/const/index.ts](file://ChatUIKit/const/index.ts#L7-L10)

### UI 层：登录、个人中心、资料与在线状态设置
- 登录页面：支持账号密码与短信验证码两种登录方式；登录成功后持久化用户标识与令牌。
- 个人中心：展示当前用户头像、名称、ID；提供退出登录入口和在线状态设置入口。
- **更新** 在线状态设置：选择预设状态（Online、Offline、Away、Busy、Do Not Disturb）或自定义描述，发布后更新用户在线状态。
- **新增** 个人资料页面：展示用户头像、昵称等基本信息，支持头像上传和昵称修改。

```mermaid
sequenceDiagram
participant M as "Me 页面"
participant AU as "AppUserStore"
participant PS as "PresenceSetting 页面"
participant PR as "Profile 页面"
participant PRSET as "ProfileSetting 页面"
participant AV as "Avatar 组件"
M->>AU : "watch(getSelfUserInfo)"
AU-->>M : "返回当前用户信息"
M->>PS : "导航到在线状态设置"
PS->>AU : "publishPresence({presenceExt})"
AU-->>PS : "发布成功"
PS-->>M : "返回个人中心"
M->>PR : "导航到个人资料"
PR->>PRSET : "导航到资料设置"
PRSET->>AU : "updateUserInfo({nickname})"
AU-->>PRSET : "更新成功"
PRSET-->>PR : "返回个人资料"
PR-->>M : "返回个人中心"
M->>AV : "传入 avatar/name/presenceExt/isOnline"
AV-->>M : "渲染头像与在线状态角标"
```

**图表来源**
- [demo/pages/Me/index.vue](file://demo/pages/Me/index.vue#L67-L88)
- [demo/pages/PresenceSetting/index.vue](file://demo/pages/PresenceSetting/index.vue#L136-L147)
- [demo/pages/Profile/index.vue](file://demo/pages/Profile/index.vue#L84-L87)
- [demo/pages/ProfileSetting/index.vue](file://demo/pages/ProfileSetting/index.vue#L58-L64)
- [ChatUIKit/components/Avatar/index.vue](file://ChatUIKit/components/Avatar/index.vue#L56-L78)

**章节来源**
- [demo/pages/Login/index.vue](file://demo/pages/Login/index.vue#L165-L302)
- [demo/pages/Me/index.vue](file://demo/pages/Me/index.vue#L67-L88)
- [demo/pages/Profile/index.vue](file://demo/pages/Profile/index.vue#L1-L116)
- [demo/pages/ProfileSetting/index.vue](file://demo/pages/ProfileSetting/index.vue#L1-L101)
- [demo/pages/PresenceSetting/index.vue](file://demo/pages/PresenceSetting/index.vue#L94-L147)
- [ChatUIKit/components/Avatar/index.vue](file://ChatUIKit/components/Avatar/index.vue#L56-L78)

### 头像组件：在线状态可视化
- 根据 isOnline 与 presenceExt 渲染不同状态图标
- 支持五种预设状态：Online、Offline、Away、Busy、Do Not Disturb
- 仅在功能开关启用时显示在线状态角标

**章节来源**
- [ChatUIKit/components/Avatar/index.vue](file://ChatUIKit/components/Avatar/index.vue#L56-L78)

## 依赖分析
- AppUserStore 依赖 ConnStore 与 ConfigStore，用于调用 SDK 与读取功能开关。
- ChatStore 协调 ConnStore、ConfigStore、AppUserStore、Conversation/Message/Contact/Group Store 完成登录后的数据初始化。
- UI 页面与组件通过 ChatUIKit 暴露的 Store API 访问用户状态。
- SDK 封装与日志工具贯穿各 Store 的调用链路。

```mermaid
graph LR
AU["AppUserStore"] --> CONN["ConnStore"]
AU --> CFG["ConfigStore"]
CS["ChatStore"] --> CONN
CS --> CFG
CS --> AU
M["Me 页面"] --> AU
PS["PresenceSetting 页面"] --> AU
PR["Profile 页面"] --> AU
PRSET["ProfileSetting 页面"] --> AU
AV["Avatar 组件"] --> AU
CS --> LOG["logger"]
AU --> LOG
```

**图表来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L12-L15)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L12-L18)
- [ChatUIKit/stores/conn.ts](file://ChatUIKit/stores/conn.ts#L10-L13)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L10-L12)
- [ChatUIKit/log.ts](file://ChatUIKit/log.ts#L1-L78)

**章节来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L12-L15)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L12-L18)
- [ChatUIKit/stores/conn.ts](file://ChatUIKit/stores/conn.ts#L10-L13)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L10-L12)
- [ChatUIKit/log.ts](file://ChatUIKit/log.ts#L1-L78)

## 性能考虑
- 智能去重：AppUserStore 在获取用户信息前会过滤已缓存 ID，避免重复请求。
- 并行与异步：登录后对用户信息与在线状态分别发起异步请求，减少阻塞。
- 功能开关：通过 ConfigStore 控制用户信息与在线状态功能，避免不必要的网络调用。
- 响应式优化：使用对象存储替代 Map，提升 Vue 响应式性能。
- **更新** 批量获取：getUsersInfoFromServer支持批量获取，减少API调用次数，提升整体性能。
- **保留** 状态发布优化：publishPresence操作直接调用SDK，避免中间状态转换开销。
- **新增** 用户信息更新优化：updateUserInfo动作自动同步更新userInfoMap，避免手动维护状态的复杂性。

**章节来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L102-L108)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L86-L125)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L24-L57)

## 故障排查指南
- 登录失败：检查登录参数与网络状态；查看 ChatStore 登录日志与错误堆栈。
- 在线状态不更新：确认是否订阅了对应用户；检查 ConfigStore 的 usePresence 开关；核对 SDK 返回的在线状态字段。
- 用户信息未显示：确认 useUserInfo 开关；检查 AppUserStore 是否正确写入 userInfoMap；确认 UI 组件是否启用在线状态显示。
- **更新** 批量获取问题：检查userIdList参数是否为空；确认去重逻辑是否正确执行；查看日志输出以定位具体问题。
- **保留** 在线状态发布失败：检查publishPresence调用参数；确认SDK连接状态；查看AppUserStore的日志输出。
- **新增** 用户信息更新失败：检查updateUserInfo调用参数；确认SDK连接状态；查看AppUserStore的日志输出；验证userInfoMap是否正确更新。
- 日志定位：通过 logger 的 info/warn/error 方法输出关键路径日志，便于定位问题。

**章节来源**
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L333-L345)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L130-L159)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L74-L75)
- [ChatUIKit/log.ts](file://ChatUIKit/log.ts#L37-L74)

## 结论
该用户状态管理方案以 Pinia Store 为核心，结合 SDK 事件与功能开关，实现了用户信息与在线状态的高效管理与可视化呈现。**更新** 通过采用fetchingUserIds去重机制和增强的getter功能，实现了更高效的用户信息管理。**新增** 通过改进updateUserInfo动作，自动同步更新userInfoMap，确保了用户信息的一致性和可靠性。**保留** 的publishPresence功能支持用户主动设置在线状态，包括五种预设状态和自定义描述，大大提升了用户体验和系统的交互性。建议在生产环境中配合严格的日志与错误监控，持续优化按需拉取与缓存策略，进一步提升用户体验与系统稳定性。

## 附录

### 用户登录与登出流程图
```mermaid
flowchart TD
A["开始"] --> B["登录页面输入凭据"]
B --> C["ChatStore.login()"]
C --> D["ConnStore.getChatConn.open()"]
D --> E["initSDKEvent()"]
E --> F["loadInitialData()"]
F --> G["AppUserStore 拉取用户信息/在线状态"]
G --> H["跳转会话列表/完成登录"]
H --> I["应用前台/后台切换"]
I --> J["ChatStore.onShow() -> ConnStore.getChatConn.onShow()"]
H --> K["退出登录"]
K --> L["ChatStore.logout()"]
L --> M["ConnStore.getChatConn.close()"]
M --> N["清空各 Store 状态"]
N --> O["回到登录页"]
```

**图表来源**
- [demo/pages/Login/index.vue](file://demo/pages/Login/index.vue#L165-L302)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L366-L371)
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L333-L361)

### 用户状态与连接状态关联关系
- 连接状态变化会触发 ChatStore 的状态更新（connected/disconnected/reconnecting），从而影响 UI 与后续数据加载。
- AppUserStore 的在线状态来源于 SDK 的订阅与查询接口，受 ConfigStore 的 usePresence 开关控制。
- **更新** 采用fetchingUserIds去重机制，通过智能过滤避免重复请求，提升状态同步效率。
- **保留** 用户可通过PresenceSetting页面主动发布在线状态，实现双向的状态管理。
- **新增** 用户信息更新通过updateUserInfo动作自动同步，确保userInfoMap与服务器状态一致。

**章节来源**
- [ChatUIKit/stores/chat.ts](file://ChatUIKit/stores/chat.ts#L74-L88)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L130-L159)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L74-L75)

### 在线状态设置与发布流程
```mermaid
sequenceDiagram
participant U as "用户"
participant PS as "PresenceSetting 页面"
participant AU as "AppUserStore"
participant SDK as "IM SDK"
U->>PS : "选择在线状态"
PS->>AU : "publishPresence({presenceExt})"
AU->>SDK : "publishPresence({description})"
SDK-->>AU : "发布成功"
AU-->>PS : "状态更新完成"
PS-->>U : "返回个人中心"
```

**图表来源**
- [demo/pages/PresenceSetting/index.vue](file://demo/pages/PresenceSetting/index.vue#L136-L147)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L187-L193)

### 用户信息更新与同步流程
```mermaid
sequenceDiagram
participant PRSET as "ProfileSetting 页面"
participant AU as "AppUserStore"
participant SDK as "IM SDK"
PRSET->>AU : "updateUserInfo({nickname})"
AU->>SDK : "updateUserInfo(params)"
SDK-->>AU : "返回更新后的用户信息"
AU->>AU : "setUserInfo(userId, res.data)"
AU-->>PRSET : "更新成功"
PRSET-->>PRSET : "页面刷新显示最新信息"
```

**图表来源**
- [demo/pages/ProfileSetting/index.vue](file://demo/pages/ProfileSetting/index.vue#L58-L64)
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L207-L222)

### 安全验证与隐私保护策略
- 凭据传递：登录采用令牌或密码方式，建议在移动端与服务端之间使用 HTTPS 与安全通道。
- 本地存储：登录成功后将用户标识与令牌写入本地存储，登出时清除；建议对敏感字段进行加密存储。
- 权限最小化：仅在必要时请求与展示用户信息与在线状态，尊重用户隐私设置。
- 功能开关：通过 ConfigStore 控制功能开关，避免在不需要的场景下采集与传输用户数据。
- **更新** 批量获取安全：采用去重机制避免重复请求，减少潜在的安全风险。
- **保留** 在线状态发布：用户可自由设置在线状态，但应提供隐私设置选项，允许用户隐藏在线状态。
- **新增** 用户信息同步安全：updateUserInfo动作自动同步更新userInfoMap，避免手动维护状态可能引入的数据不一致问题。

**章节来源**
- [demo/pages/Login/index.vue](file://demo/pages/Login/index.vue#L177-L183)
- [demo/pages/Me/index.vue](file://demo/pages/Me/index.vue#L82-L87)
- [ChatUIKit/stores/config.ts](file://ChatUIKit/stores/config.ts#L24-L57)

### 用户信息获取优化流程
```mermaid
flowchart TD
Start(["用户信息获取请求"]) --> CheckFeature["检查 useUserInfo 开关"]
CheckFeature --> CheckEmpty{"用户ID列表是否为空？"}
CheckEmpty --> |是| ReturnEmpty["返回空结果"]
CheckEmpty --> |否| FilterCache["过滤已缓存用户ID"]
FilterCache --> CheckRemaining{"是否有待获取用户？"}
CheckRemaining --> |否| ReturnCache["返回已缓存结果"]
CheckRemaining --> |是| CallSDK["调用 SDK fetchUserInfoById"]
CallSDK --> UpdateState["更新 userInfoMap"]
UpdateState --> ReturnResult["返回获取结果"]
ReturnEmpty --> End(["结束"])
ReturnCache --> End
ReturnResult --> End
```

**图表来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L86-L125)

### 用户信息更新优化流程
```mermaid
flowchart TD
Start(["用户信息更新请求"]) --> CheckFeature["检查 useUserInfo 开关"]
CheckFeature --> CallSDK["调用 SDK updateUserInfo"]
CallSDK --> GetUserId["获取当前用户ID"]
GetUserId --> UpdateState["调用 setUserInfo 更新 userInfoMap"]
UpdateState --> ReturnResult["返回更新结果"]
ReturnResult --> End(["结束"])
```

**图表来源**
- [ChatUIKit/stores/appUser.ts](file://ChatUIKit/stores/appUser.ts#L207-L222)