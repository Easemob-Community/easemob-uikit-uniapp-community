...
**更新** 本次更新重点增强了头像显示逻辑的重大增强，包括远程用户和群组信息获取、智能回退机制、自动用户信息获取功能等。会话项组件现在使用增强的群组状态管理getter来正确显示群组名称和头像，支持字段兼容性和双重缓存机制，提供更好的数据获取性能和可靠性。**新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示，支持presenceExt和isOnline字段，为单聊会话提供完整的在线状态显示。
...
**更新** 新增了增强的头像显示逻辑，包括智能回退机制、自动用户信息获取功能，以及增强的群组状态管理getter，支持字段兼容性和双重缓存机制。**新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示，支持presenceExt和isOnline字段。
...
**更新** 头像显示逻辑现在使用增强的 `conversationInfo` 计算属性，提供智能回退机制和更好的错误处理。**新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示。
...
**新增** 在线状态指示器功能为单聊会话提供了完整的在线状态显示，通过showPresenceIndicator计算属性智能控制显示逻辑，支持presenceExt和isOnline字段的完整状态管理。Avatar组件接收withPresence、presenceExt、isOnline新属性，提供完整的在线状态指示器渲染。
