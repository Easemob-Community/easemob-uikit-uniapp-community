...
- **新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示，支持presenceExt和isOnline字段
- **更新** 头像组件Avatar接收withPresence、presenceExt、isOnline新属性，提供完整的在线状态显示
- **新增** 表情符号渲染功能，确保表情符号在会话列表中正确显示，支持Unicode表情和中文表情shortcode
- **更新** 会话项组件集成renderTxt函数，提供完整的表情符号处理和显示能力
...
**更新** 本次更新重点增强了头像显示逻辑的重大增强，包括远程用户和群组信息获取、智能回退机制、自动用户信息获取功能等。会话项组件现在使用增强的群组状态管理getter来正确显示群组名称和头像，支持字段兼容性和双重缓存机制，提供更好的数据获取性能和可靠性。**新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示，支持presenceExt和isOnline字段，为单聊会话提供完整的在线状态显示。**新增** 表情符号渲染功能，确保表情符号在会话列表中正确显示，支持Unicode表情和中文表情shortcode的实时处理和显示。
...
**图表来源**
- [ConversationList/index.vue:1-158](file://ChatUIKit/modules/Conversation/components/ConversationList/index.vue#L1-L158)
- [ConversationItem/index.vue:1-260](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L1-L260)
...
- [ChatNav/index.vue:1-135](file://ChatUIKit/modules/Chat/components/ChatNav/index.vue#L1-L135)
- [index.ts:1-344](file://ChatUIKit/utils/index.ts#L1-L344)
- [emoji.ts:1-127](file://ChatUIKit/const/emoji.ts#L1-L127)
...
**更新** 新增了增强的头像显示逻辑，包括智能回退机制、自动用户信息获取功能，以及增强的群组状态管理getter，支持字段兼容性和双重缓存机制。**新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示，支持presenceExt和isOnline字段。**新增** 表情符号渲染功能，通过renderTxt函数提供完整的表情符号处理和显示能力。
...
- 数据绑定
  - 头像：根据会话类型选择用户或群组头像，支持占位符与错误回退，**新增** 支持在线状态指示器。
...
**更新** 头像显示逻辑现在使用增强的 `conversationInfo` 计算属性，提供智能回退机制和更好的错误处理。**新增** 在线状态指示器功能，通过showPresenceIndicator计算属性控制显示。**新增** 表情符号渲染功能，通过renderTxt函数提供完整的表情符号处理。

**章节来源**
- [ConversationItem/index.vue:1-260](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L1-L260)
- [ConversationItem/style.scss:1-152](file://ChatUIKit/modules/Conversation/components/ConversationItem/style.scss#L1-L152)

### 头像显示与在线状态指示器

**重大更新** 会话项组件现在具有增强的头像显示逻辑，包括远程用户和群组信息获取、智能回退机制、自动用户信息获取功能，以及完整的在线状态指示器支持。
...
- **新增** **presenceExt和isOnline字段**：为单聊会话提供在线状态信息

#### 在线状态指示器功能

**新增功能** 会话项组件现在支持完整的在线状态指示器功能：
...
这个 `showPresenceIndicator` 计算属性控制在线状态指示器的显示：

- **功能开关**：通过featureConfig.usePresence控制是否启用在线状态功能
- **会话类型限制**：仅在单聊会话中显示在线状态指示器
- **智能判断**：自动根据会话类型和功能配置决定是否显示
...
**更新** 头像组件Avatar现在接收以下新属性：

- **withPresence**：布尔值，控制是否显示在线状态指示器
- **presenceExt**：字符串，表示用户的在线状态扩展信息（如"Online"、"Away"等）
- **isOnline**：布尔值，表示用户是否在线

这些属性通过showPresenceIndicator计算属性和conversationInfo计算属性传递给Avatar组件。

**章节来源**
- [ConversationItem/index.vue:134-144](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L134-L144)
- [ConversationItem/index.vue:162-168](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L162-L168)
- [Avatar/index.vue:29-44](file://ChatUIKit/components/Avatar/index.vue#L29-L44)
- [Avatar/index.vue:56-58](file://ChatUIKit/components/Avatar/index.vue#L56-L58)
- [Avatar/index.vue:60-78](file://ChatUIKit/components/Avatar/index.vue#L60-L78)
- [appUser.ts:35-46](file://ChatUIKit/stores/appUser.ts#L35-L46)
- [group.ts:68-95](file://ChatUIKit/stores/group.ts#L68-L95)
- [index.ts:10-12](file://ChatUIKit/const/index.ts#L10-L12)
...
**章节来源**
- [ConversationItem/index.vue:213-215](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L213-L215)
- [index.ts:275-312](file://ChatUIKit/utils/index.ts#L275-L312)
- [conversation.ts:94-97](file://ChatUIKit/stores/conversation.ts#L94-L97)
...
**章节来源**
- [ConversationItem/style.scss:7-9](file://ChatUIKit/modules/Conversation/components/ConversationItem/style.scss#L7-L9)
- [ConversationItem/index.vue:130-132](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L130-L132)
- [ConversationItem/index.vue:182-188](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L182-L188)
...
**图表来源**
- [ConversationItem/index.vue:232-251](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L232-L251)

**章节来源**
- [ConversationItem/index.vue:202-230](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L202-L230)
- [ConversationList/index.vue:95-97](file://ChatUIKit/modules/Conversation/components/ConversationList/index.vue#L95-L97)
...
**章节来源**
- [index.ts:275-312](file://ChatUIKit/utils/index.ts#L275-L312)
- [ConversationItem/index.vue:40-63](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L40-L63)
...
**章节来源**
- [index.ts:226-265](file://ChatUIKit/utils/index.ts#L226-L265)
- [emoji.ts:55-127](file://ChatUIKit/const/emoji.ts#L55-L127)
...
**章节来源**
- [ConversationItem/index.vue:107-113](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L107-L113)
- [ConversationItem/style.scss:1-152](file://ChatUIKit/modules/Conversation/components/ConversationItem/style.scss#L1-L152)
...
**图表来源**
- [ConversationItem/index.vue:208-210](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L208-L210)
- [index.vue:229-245](file://ChatUIKit/modules/Chat/index.vue#L229-L245)

**章节来源**
- [ConversationItem/index.vue:202-210](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L202-L210)
- [index.vue:211-219](file://ChatUIKit/modules/Chat/index.vue#L211-L219)

### 表情符号渲染与显示优化

**新增功能** 会话项组件现在集成了完整的表情符号渲染系统，确保表情符号在会话列表中正确显示。

#### renderTxt 函数实现
会话项组件通过 `renderTxt` 函数处理表情符号渲染：
...
>
  <span
    v-if="conversation.conversationType === 'groupChat' && !conversation.lastMessage?.noticeInfo"
...
该函数支持以下特性：
- **Unicode表情支持**：支持U+1F600、U+1F604等Unicode格式的表情
- **中文表情支持**：支持[微笑]、[开心]等中文表情shortcode
- **混合文本处理**：智能分割文本和表情，分别渲染
- **样式适配**：通过emoji-wrap类名和.msg-emoji样式确保表情正确显示

#### 样式系统支持
会话项组件使用以下样式确保表情符号正确显示：
...
该映射系统支持：
- **表情URL映射**：每个Unicode表情对应具体的图片URL
- **中文表情映射**：提供中文表情到Unicode表情的双向映射
- **替代文本支持**：为每个表情提供替代文本描述

**章节来源**
- [ConversationItem/index.vue:40-63](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L40-L63)
- [ConversationItem/index.vue:52-59](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L52-L59)
- [ConversationItem/style.scss:107-109](file://ChatUIKit/modules/Conversation/components/ConversationItem/style.scss#L107-L109)
- [common.scss:14-17](file://ChatUIKit/styles/common.scss#L14-L17)
- [index.ts:226-265](file://ChatUIKit/utils/index.ts#L226-L265)
- [emoji.ts:55-127](file://ChatUIKit/const/emoji.ts#L55-L127)
...
**章节来源**
- [ConversationItem/index.vue:94-105](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L94-L105)
- [ConversationList/index.vue:33-41](file://ChatUIKit/modules/Conversation/components/ConversationList/index.vue#L33-L41)
...
- **新增** 用户信息获取优化
  ...
  - 错误处理机制确保系统的稳定性
- **新增** 在线状态指示器性能优化
  - `showPresenceIndicator` 计算属性提供智能判断，避免不必要的DOM渲染
  - presenceExt和isOnline字段的响应式更新，确保状态变化时及时反映
  - Avatar组件的条件渲染，只有在需要时才显示在线状态指示器
- **新增** 表情符号渲染性能优化
  - renderTxt函数使用高效的正则表达式匹配
  - 表情映射表使用对象查找，O(1)时间复杂度
  - 样式系统优化，避免重复的DOM操作

**章节来源**
- [conversation.ts:49-98](file://ChatUIKit/stores/conversation.ts#L49-L98)
- [index.ts:107-134](file://ChatUIKit/utils/index.ts#L107-L134)
- [Avatar/index.vue:56-58](file://ChatUIKit/components/Avatar/index.vue#L56-L58)
- [Avatar/index.vue:89-95](file://ChatUIKit/components/Avatar/index.vue#L89-L95)
- [group.ts:68-95](file://ChatUIKit/stores/group.ts#L68-L95)
- [appUser.ts:86-125](file://ChatUIKit/stores/appUser.ts#L86-L125)
...
- 在线状态异常
  ...
  - **新增** 验证 showPresenceIndicator 计算属性的逻辑是否正确。
  - **新增** 检查 Avatar 组件的 withPresence、presenceExt、isOnline 属性传递。
- 未读数不更新
...
- **新增** Emoji渲染问题
  ...
  - 验证表情图片资源是否正确加载。
  - **新增** 检查会话项模板中的表情渲染逻辑是否正确。
  - **新增** 验证样式系统是否正确应用emoji-wrap和.msg-emoji类名。
- **新增** 在线状态指示器问题
  - 检查 featureConfig.usePresence 配置是否正确。
  - 确认会话类型判断逻辑是否正确（群聊不应显示在线状态）。
  - 验证 presenceExt 和 isOnline 字段的数据来源和格式。
  - 检查 Avatar 组件的在线状态样式是否正确加载。

**章节来源**
- [appUser.ts:86-125](file://ChatUIKit/stores/appUser.ts#L86-L125)
- [group.ts:136-167](file://ChatUIKit/stores/group.ts#L136-L167)
- [conversation.ts:246-276](file://ChatUIKit/stores/conversation.ts#L246-L276)
- [index.ts:275-312](file://ChatUIKit/utils/index.ts#L275-L312)
- [ConversationItem/index.vue:232-251](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L232-L251)
...
**更新** 新增的增强头像显示逻辑显著提升了组件的可靠性和用户体验。通过智能回退机制、自动用户信息获取功能和增强的群组状态管理getter，组件现在能够更好地处理各种数据获取场景，提供更稳定的服务。同时，新增的emoji渲染功能进一步提升了用户体验，支持多种表情格式的实时处理和显示。**新增** 在线状态指示器功能为单聊会话提供了完整的在线状态显示，通过showPresenceIndicator计算属性智能控制显示逻辑，支持presenceExt和isOnline字段的完整状态管理。Avatar组件接收withPresence、presenceExt、isOnline新属性，提供完整的在线状态指示器渲染。

对于高级用户，可通过 FeatureConfig 与样式覆盖实现灵活定制与扩展，同时可以利用新增的emoji功能、增强的头像显示逻辑、改进的群组状态管理和在线状态指示器进行更精细的文本处理、显示优化、数据获取优化和状态管理。
...
- **新增** Emoji表情使用
  - 支持Unicode格式：如 😄、😀、😉 等
  - 支持中文表情：如 [微笑]、[开心]、[惊讶] 等
  - 在会话列表中正确显示表情符号，提升消息预览的可读性
- **新增** 群组信息显示
  - 群组会话自动使用群组名称和头像
  - 支持多种字段格式的兼容显示（groupName/avatarUrl vs groupname/avatarurl）
- **新增** 在线状态指示器使用
  - 单聊会话自动显示在线状态指示器
  - 支持多种在线状态：Online、Away、Busy、Do Not Disturb等
  - 通过presenceExt和isOnline字段提供完整状态信息

**章节来源**
- [ConversationList/index.vue:16-26](file://ChatUIKit/modules/Conversation/components/ConversationList/index.vue#L16-L26)
- [ConversationItem/index.vue:202-210](file://ChatUIKit/modules/Conversation/components/ConversationItem/index.vue#L202-L210)
...
- 性能优化
  ...
  - **新增** 在线状态指示器优化：使用计算属性智能控制显示，避免不必要的DOM渲染。
  - **新增** 表情符号渲染优化：使用高效的正则表达式匹配和对象查找，优化渲染性能。
- 扩展方案
  ...
  - **新增** 自定义在线状态：支持自定义presenceExt状态和对应的视觉样式。
  - **新增** 在线状态订阅：通过subscribePresence和unsubscribePresence管理用户在线状态订阅。
  - **新增** 表情符号样式定制：支持自定义表情符号的尺寸、间距和对齐方式。
...
- [group.ts:68-95](file://ChatUIKit/stores/group.ts#L68-L95)
- [appUser.ts:86-125](file://ChatUIKit/stores/appUser.ts#L86-L125)
- [appUser.ts:164-182](file://ChatUIKit/stores/appUser.ts#L164-L182)