...
export default {
  ...
  methods: {
    ...
    editMessage() {
      console.log('[MessageActions] editMessage called, msg:', this.msg.id, 'serverMsgId:', this.msg.serverMsgId)
      ...
      console.log('[MessageActions] Dispatching setEditingMessage')
    ...
    deleteMessage() {
      const convId = this.msg.chatType === 'groupChat' ? this.msg.to : 
        (this.msg.from === (this.$store.getters['conn/getChatConn'] && this.$store.getters['conn/getChatConn'].user) ? this.msg.to : this.msg.from)
